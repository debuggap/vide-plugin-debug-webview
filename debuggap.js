	...
	*/
	...
	ScriptSocket.prototype = {
		...
		error:function(){
			...
			if( this.currentTimes++ != this.tryMaxTimes ){
				...
				delete localStorage.scriptSocket;
	...
	dg.scriptSocket = {
		...
		handle:function(data){
			socket && socket.onmessage({data:data});
		}
	}
	...
	function calculateCss(a) {
		...
		function _calculateStyleRule(a,rule){
			...
			if( a.matches(rule.selectorText) ){
				var arr = splitCSS(rule.selectorText),matchSelector=rule.selectorText;
				for( var j=0;j<arr.length;j++){
					if( a.matches(arr[j]) ){
						matchSelector=arr[j];
						break;
				...
				result = {css:rule.cssText,selectors:rule.selectorText,selector:matchSelector};
			}
			return result;
		}
		...
		XMLHttpRequest.prototype.open = function(method,url,async){
			try{
				...
				if( str.slice(0,4) == 'http' || str.slice(0,4) == 'file' ){
					...
					xmlObj[uniqueId] = {method:method,url:str,header:{}}
				}
			}catch(e){}
	...
	dg.init ={
		...
		addWrap:function(){
			...
			if( dg.browser == 'ie' ){
				dg.css(qs('#debuggapBlock'),{display:'none'});
			}
		},
		addStyle:function(){
			...
			'#debuggapRoot .dg-out{background-color: transparent;position: absolute;z-index: 999;top: 20px;right: 20px;border: 2px solid #00abe3;border-radius: 30px;width: 30px;height: 30px;box-sizing: content-box;}'+
	...
	var job ={
		...
		_getTagAndAttr:function(ele){
			var obj = {t:ele.nodeName.toLowerCase(),c:ele.childNodes.length>0 ? []:false};
		...
		_doCacheJs:function(filePath,con){
			...
			qs('#debuggapRoot').appendChild(dg.createEle('script',{_src:filePath},con));
		},
		_doCacheCss:function(filePath,con){
			...
			styleTag.parentNode.insertBefore(dg.createEle('style',{_href:filePath},con),styleTag);
			styleTag.remove();
		},
		...
		doGetPrompt:function(str){
			...
			for( var i=0;i<len-1;i++){
				try{
					...
					send('prompt:'+timestamp+':'+JSON.stringify({msg:e.message}));
				}
			}
	...
	dg.extend({
		...
		_getCurrentAddr:function(){
			var rt=[];
			var scripts = document.scripts,result;
		...
		initSocket:function(conf){
			...
			var useScriptSocket = 0;
		    try{
				if( localStorage.scriptSocket == 1 ){
					//use script socket to communicate
					useScriptSocket = 1;
				}else{
	    	...
	    	if( useScriptSocket || !socket){
	        	socket = new ScriptSocket(conf);
				this.scriptSocketFlag = 1;
				localStorage.scriptSocket == 1 
	        }
